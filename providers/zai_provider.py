...
class ZaiProvider(BaseProvider):
    ...
    def get_usage_data(self, period: str = 'daily') -> Dict[str, Any]:
        """
        Get Z.ai usage data for specified period
        ...
        try:
            ...
            logger.error(f"Error getting Z.ai usage data: {e}")
            return {
                "provider": "zai",
                "error": str(e),
                "usage": {}
    ...
    def check_rate_limit(self) -> bool:
        """
        Check if Z.ai rate limits are not exceeded
    ...
    def get_current_usage(self) -> Dict[str, Any]:
        """
        Get current Z.ai usage statistics
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Z.ai API error: {response.status_code}")
                return {
                    'tokens': 0,
                    'requests': 0,
                    'cost': 0,
            ...
            logger.error(f"Error getting Z.ai current usage: {e}")
            return {
                'tokens': 0,
                'requests': 0,
                'cost': 0,
    ...
    def get_usage_history(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """
        Get Z.ai usage history for date range
        ...
        try:
            ...
            logger.error(f"Error getting Z.ai usage history: {e}")
            return {
                "provider": "zai",
                "error": str(e),
                "data": []
