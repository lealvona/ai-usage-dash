...
class OpenAIProvider(BaseProvider):
    ...
    def get_usage_data(self, period: str = 'daily') -> Dict[str, Any]:
        """
        Get OpenAI usage data for specified period
        ...
        """
        try:
            # OpenAI doesn't provide historical usage in the API
            # We'll return mock data based on configuration
    ...
    def check_rate_limit(self) -> bool:
        """
        Check if OpenAI rate limits are not exceeded
    ...
    def get_current_usage(self) -> Dict[str, Any]:
        """
        Get current OpenAI usage statistics
        ...
        try:
            ...
            logger.error(f"Error getting OpenAI current usage: {e}")
            return {
                'tokens': 0,
                'requests': 0,
                'cost': 0,
    ...
    def get_usage_history(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """
        Get OpenAI usage history for date range
        ...
        """
        try:
            # OpenAI API doesn't provide historical usage
            # Return mock historical data based on configuration
            return {
                "provider": "openai",
                "period": "custom",
            ...
            logger.error(f"Error getting OpenAI usage history: {e}")
            return {
                "provider": "openai",
                "error": str(e),
                "data": []
